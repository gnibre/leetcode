class Solution(object):
    def minSubArrayLen(self, s, nums):
        ...
        # 2,5,6,8,12,15
        # sorted,
        # pick anytwo, have - > 7, as close as possible.
        # # binary search?
        # [ s1, ......sm,    ...... sn]
        ...
        #  ln => total  sufficient?
        #  ln/2  also sufficient?  there's n/2 of them.
        #  ln/4 ?  (n-n/4) of them
        #  after all. should be able to find it with o(nlogn)
        
        
        rs = []
        sm = 0
        sp = -1  # min value for sequence end.
        for x in nums:
            if x>=s:
                return 1
            sm+=x
        ...
        for ed in xrange(sp,len(nums)):
            #  find biggest st position to have  sum[st to ed] >= given s
            #  biggest st to make sequence shorter
            # when start position(st) is given, 
            st = max(-1, ed-cur_min+1)

            p = self.findMaxStartPosition(rs,st,ed,s)

            print " try from  idx[%d,%d] v: [%d,%d] , best p: %d -> v %d" % (st,ed,rs[st],rs[ed],p, rs[p])
            # print "ed-p :  %d " % (ed-p) 
            if p>-2:
                # l = ed-p+1
                if ed-p<cur_min:
                    cur_min = ed-p
            print "cur min: %d " %cur_min
            
        
        return cur_min
        
        
        #  find biggest p to satisfy  rs[ed]-rs[p] > s
        # also p > s
        
    def findMaxStartPosition(self, rs,st,ed,s):
        # print "-> find [%d,%d]   " % (st,ed)
        # 
        ...
        while True:
            if up-low<10:
                for x in xrange(low,up+1):
                    rsum = rs[ed]-rs[x]
                    if rsum>=s and x>maxp:
                        maxp = x
                break
